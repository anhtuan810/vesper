...
const ALLOWED_CHIPS: ReadonlySet<string> = new Set([
  "Confirm and save",
...
function sanitizeChips(raw: unknown): string[] | null {
  ...
  // Reject placeholder patterns defensively in case allowlist
  // is widened later: anything containing $, [, ], <, > is out.
...
async function resolveProposal(proposal: ProposalChange, currentAssets: CurrentAssetLight[]): Promise<string> {
  ...
  if (action === "edit" && typeof proposal.value_delta === "number") {
    // value_delta path — resolve units delta from live price
    const existing = currentAssets.find(
      (a) => a.name.toLowerCase() === name.toLowerCase() ||
             (a.symbol && a.symbol.toLowerCase() === name.toLowerCase())
    );
  ...
  if (action === "add") {
    ...
    // Explicit-units add (possibly with inferred buy_date)
  ...
  // Fallback for edit with explicit units (no value_delta)
