...
- **Optimistic chat messages survive error paths.** When the chat assistant fails (timeout, 500, rate-limit error), the user sees their message and an error reply in the UI, but neither is persisted to the database. Reloading the page makes both disappear. Acceptable for MVP — the user can simply retry. See `use-chat-session.ts` `send()` for the optimistic-add pattern.
