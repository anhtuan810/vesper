...
- **Worth knowing** and **Markets** share one presentation, `SwipeExpandCarousel`: a leading icon (bulb / activity), a full-width swipe carousel of up to 3 collapsible `{title, detail}` slides — italic serif headline + chevron, tap to expand the dim ~12.5px detail below — and inline tappable dots. The carousel's height animates to the active slide's height (collapsed vs. expanded). Borderless: no box, border, background, or shadow.
  - **Worth knowing** (`InsightBand`): up to 3 ordered `{title, detail}` cards from `/api/insight`.
    - Deterministic detectors (`detectConcentration`, `detectCurrencyMismatch`, `detectCashDrag` in `src/lib/portfolio-insights.ts`, in that order) each have a deterministic title/detail fallback; up to 3 fired detectors are batched into one Haiku call that phrases both fields, validated against the deterministic figures (numeric-integrity guardrail) and a title constraint (short, no numbers) — falling back per-field if either drifts.
    - **Thin-portfolio handling**: portfolios of 1–3 assets skip the detectors and Haiku — `generateInsight` (`src/lib/insight-generator.ts`) returns deterministic `{title, detail}` copy naming the held positions and the most common absent categories (cash, pension, property). Portfolios of 4+ assets with no detector firing fall back to a Haiku two-sentence observation, also split into `{title, detail}`. Cost: ~$0.0001–0.0003 per Haiku call (waived for thin portfolios).
    - Cached 24h in the `highlights` table — `type='portfolio'` for detector cards, `type='insight'` for the legacy fallback, both with `title` and `detail` populated. No new schema.
    - `/api/insight` GET returns `{ insights: [{title, detail}], insight: {detail, expires_at?}, market }`; `insight.detail` is kept for backward compatibility.
    ...
    - Tapping an expanded detail stores a seed in `sessionStorage` and navigates to `/chat?seed=insight&key=current`.
  - **Markets** (`MarketsHighlights`): up to 3 daily market-news items (cron-generated `type='market'` highlights, deserialized via `parseMarketDetail`), mapped to `{title: headline, detail: body}`. Read-only — no chat hand-off. Renders nothing when there are no current market highlights.
- Files: `src/components/PortfolioSummaryCard.tsx`, `src/components/scenario/ProjectionTeaser.tsx`, `src/components/InsightBand.tsx`, `src/components/MarketsHighlights.tsx`, `src/components/SwipeExpandCarousel.tsx`, `src/components/SwipeCarousel.tsx` (`CarouselDots`), `src/app/api/insight/route.ts`, `src/lib/insight-generator.ts`, `src/lib/portfolio-insights.ts`, `src/lib/market-highlights.ts`, `src/app/api/cron/market-highlights/route.ts`, `src/app/api/dashboard-init/route.ts`, `src/lib/hooks/insight.ts` (`useInsight()`)
