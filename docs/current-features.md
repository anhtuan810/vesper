...
- **No avatar on Profile.** `users.avatar_url` remains in the schema and `/api/users/me` accepts it (Google OAuth on signup still writes it), but no UI surface renders it. `src/lib/avatar-upload.ts` is unused.
