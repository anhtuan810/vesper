...
- **A snapshot may never predate the current book** (2026-08-09). One invariant, enforced in one place: every historical row is recomputed from the assets on the book *now*, so a row older than `earliest` — the first date that book reaches — cannot be recomputed and cannot be right. It can only be residue of a holding that was **erased** (a hard-deleted "mistake"; a *sold* asset keeps its row, so it keeps its dates inside `earliest`). `backfillSnapshots` therefore deletes `date < earliest` on **both** passes, gated on a non-empty recompute so a transient all-null valuation can't turn the sweep into "delete everything and write nothing".
  - **What this fixes.** The rebuild's delete window is clamped up to `earliest` so it never deletes more than it can replace (`rebuildStart = max(rebuildFrom, earliest)`) — which meant removing the **oldest** holding stranded every year that holding alone occupied. Those rows survived verbatim, still carrying the removed asset's band, while the freshly rebuilt history began years later: the reported "removed the house and the graph is a total mess" — a multi-year house-shaped hump ending in a cliff, with nothing in the book to explain it, and the same period reading as a flat near-zero line under the Liquid lens.
  - **Self-heal.** `/api/cron/snapshot` runs the rebuild not only when a user has no pre-today history, but also when their **oldest snapshot predates `bookFloorDate(userId)`** — the earliest date the current book can date itself from (asset `created_at`, normalised `buy_date`, earliest dated mutation). Including `buy_date` can only pull that floor *earlier* than the rebuild's own `earliest`, which is the safe direction: too early merely fails to notice residue (the next rebuild sweeps it anyway); too late would flag healthy history and re-rebuild it daily. A null floor (nothing to date the book from) is never read as "it's all residue". This is what repairs accounts already carrying the residue — no manual script run.
  - **The provisional line matches.** `reconcileHistoryToHoldings` (`networth-estimate.ts`) drops the **leading** points a removal empties out, instead of flattening them to the floor — same shape the rebuild will land on, so the swap stays seamless. A point only qualifies if the removal is what emptied it (above the 1%-of-peak floor before, below it after), so a user who genuinely started at €800 and grew to €200k keeps their early history. Leading only (an interior hole is a real gap, and the line should span it) and the last point is never trimmed, so a series can't collapse to nothing. Covered in `verify-networth-estimate.ts`.
