...
### market_stories
Global cache of the **"story behind"** an auto-logged market swing: one short, search-grounded sentence explaining *why* a symbol moved on a given date (e.g. "Prices slid as the Fed signalled rates would stay higher for longer."). The templated `market_swings` entry already states the up/down move and the € impact — this adds the *reason*, rendered as a clause in the full-card entries (`MobileMarketEntry`, `DesktopMarketEntry`; asset cards, which had no prose, now show the story as their read of the day). Not user-scoped — the reason a market/asset moved on a date is the SAME for everyone, so one row per (date, symbol) serves all users (same global-reference shape as `price_history` / `fx_rate_history` / `market_moves`; RLS enabled, no policies). `src/lib/market-story-cache.ts`: the read path `attachStories` joins the story in by (date, `index_symbol`); `backfillMarketStories` generates the missing ones in the **background** (`after()` on `GET /api/diary/market-moves`, bounded per pass, asset-relevant entries first), grounded by `web_search` on `claude-sonnet-4-6` (same model + tool as `market-highlights.ts`). Because the story is global and generated off the hot path, the per-user swing rebuild (`generateMarketSwings`) stays pure price/FX math and is **not slowed**; across the whole user base only a handful of new stories are generated a month. A row's *existence* means "already looked up": `story` NULL means no confident cause was found (a noisy day — keep the plain entry, don't retry); non-NULL is the sentence. A row is written only after a completed lookup, so a transient failure is retried, never cached as "no story". Best-effort throughout — a missing table (pre-migration) or any error degrades to entries rendering without a story, so it is **safe to deploy before the migration is applied**. PK (date, symbol). Migration `20260709_market_stories.sql`. **The feature is currently OFF, gated behind the `MARKET_STORIES_ENABLED` env flag (`marketStoriesEnabled()` in `market-story-cache.ts`).** It was disabled after a cost incident: while the migration was unapplied, `readStoryRows` couldn't tell "table missing" from "cache empty", so `backfillMarketStories` generated up to `DEFAULT_BACKFILL_LIMIT` stories per diary view and `storeStory` silently failed to persist them — regenerating on every view, an unbounded Sonnet + web-search bill. The read now distinguishes cache-unavailable (`{ ok:false }`) from cache-empty and never generates a story it can't persist; `web_search` is capped at `max_uses: 3` per call; and both `attachStories` and `backfillMarketStories` short-circuit when the flag is off. Re-enabling requires **both** `MARKET_STORIES_ENABLED="true"` and the migration applied.
