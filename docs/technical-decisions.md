...
- **The history rebuild has ONE mode: rebuild everything** (2026-08-09). `backfillSnapshots(userId)` recomputes the user's entire reconstructed history from the book as it stands now and atomically replaces every row before today (delete + insert; `writeSnapshot` owns today's row). There is no "from this date forward" variant and no `rebuildFrom` parameter. Callers decide only *whether* to rebuild, never from when — `applyPortfolioChanges` returns `touchesHistory: boolean` (was `rebuildFrom: string | null`, a date used as a flag) and the agent loop carries it as `needsBackfill`.
  - **Why the scoped write was removed.** It bought nothing. Every expensive input is scoped by `earliest` (the first date the current book reaches), not by the caller's date: each symbol's price history (`getPriceSeriesCached` from `earliestHeldBySymbol`), the multi-year FX series, the property samplers, the mortgage balance points, the date lattice, and the per-date `computeRow` loop all ran in full either way. `rebuildFrom` only split rows that had *already been computed, seconds earlier*, into "write" and "leave alone" — a few hundred row-writes saved, in exchange for a whole class of bug.
  - **The bug it cost.** The scoped delete was clamped so it never outran what it could replace (`rebuildStart = max(rebuildFrom, earliest)`), which left the window *below* `earliest` with no owner at all. Removing the **oldest** holding therefore stranded every year that holding alone occupied: those rows survived verbatim, still carrying the removed asset's band, in front of a rebuilt history that now began years later. The reported "removed the house and the graph is a total mess" — a multi-year house-shaped hump ending in a cliff, with nothing in the book to explain it, reading as a flat near-zero line under the Liquid lens. Rebuilding the whole range makes this structurally impossible rather than something a guard has to catch.
  - **Recovery is now unconditional.** A run degraded by a market-data outage (a held symbol valued at cost basis, see `missingHeld`) is fully corrected by the *next* run, whatever triggered it. Under scoped rebuilds a degraded slice could persist indefinitely, because nothing was ever going to rebuild that particular range again. The "never write nothing" guard is unchanged: an empty recompute skips the delete entirely, so delete and insert only ever happen together.
  - **Off-lattice rows are preserved.** Pre-existing rows at dates the sparse lattice doesn't target (older daily-cron rows) are recomputed and reinserted, so replacing the range never silently thins the chart. Rows older than `earliest` are deliberately *not* recomputed — that is what lets the delete carry away residue from an erased holding.
  - **Self-heal for accounts already carrying residue.** `/api/cron/snapshot` rebuilds not only when a user has no pre-today history, but also when their oldest snapshot predates `bookFloorDate(userId)` — the earliest date the current book can date itself from (asset `created_at`, normalised `buy_date`, earliest dated mutation). Including `buy_date` can only pull that floor *earlier* than the rebuild's own `earliest`, the safe direction: too early merely fails to notice residue (the next rebuild clears it anyway); too late would flag healthy history and re-rebuild it daily. A null floor is never read as "it's all residue". The condition stops firing once one rebuild has run.
  - **The provisional client line matches.** `reconcileHistoryToHoldings` (`networth-estimate.ts`) drops the **leading** points a removal empties out instead of flattening them onto the floor — previously a removal could only lower a point, never drop it, so the estimate always drew a multi-year flatline ending in a wall. A point only qualifies if the removal is what emptied it (above the 1%-of-peak floor before, below it after), so a user who genuinely started at €800 and grew to €200k keeps their early history. Leading only (an interior hole is a real gap the line should span), and the last point is never trimmed, so a series can't collapse to nothing. Covered in `verify-networth-estimate.ts`.
...
- **Latency architecture** (the demo lives on this gesture):
  ...
  - **Records-stamp invalidation**: caches key on `mutations.length + newest
    recorded_at` — the same events that trigger `backfillSnapshots`.
...
- ~~**No analytics**~~ — **shipped**: `@vercel/analytics` with 5 pilot events (signup, first_asset_added — now carrying `minutes_since_signup`, first_chat_mutation, return_visit_day2_plus, demo_verdict_seen) plus chip impression/interaction telemetry (`src/lib/chip-telemetry.ts`) and the demo funnel below. See "The First 60 Seconds" above for the two hook metrics.
- **Demo funnel events** (2026-08, web only — the native build mounts no `<Analytics />`). Three events, all gated on the `isDemo` entitlement so they never fire for a real signed-in account: `demo_started` (the `/demo` route appends `?demo=1` to its SUCCESS redirect only, so a rate-limited or failed entry — which redirects to `/login` — is not counted; the dashboard strips the param and fires once the entitlement resolves), `demo_message_sent` with a 1-based `message_index` (every send path in `use-chat-session.ts`, so per-turn drop-off across the `DEMO_CHAT_DAILY_LIMIT` allowance is visible), and `demo_signup_click` with a `placement` prop (banner / expiry_wall / chat_wall / settings / desktop_settings) on every CTA that leaves the demo for account creation. No PII on any of them. `demo_started` covers the legacy shared-account demo too — that account is also `isDemo`, so the funnel is measurable in production today rather than only once `DEMO_ENABLED` is switched on.
