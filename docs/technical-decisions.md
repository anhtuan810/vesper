...
- Subscriptions — RevenueCat (mobile): `NEXT_PUBLIC_REVENUECAT_IOS_KEY`, `NEXT_PUBLIC_REVENUECAT_ANDROID_KEY` (Android, later), `NEXT_PUBLIC_REVENUECAT_ENTITLEMENT_ID`, `REVENUECAT_WEBHOOK_AUTH`, `REVENUECAT_MONTHLY_PRODUCT_ID`, `REVENUECAT_ANNUAL_PRODUCT_ID`, `REVENUECAT_ALLOW_SANDBOX` (staging/dev only — never `true` in production), `REVENUECAT_SECRET_API_KEY` (optional; server-only — enables RevenueCat-side customer deletion on account deletion **and** deterministic entitlement reconcile/self-heal in `GET /api/subscription`)
...
- **Deterministic activation (self-heal), not webhook-dependent**: webhooks are pushed and eventually-consistent (they can be delayed, dropped, or sandbox-gated). So `GET /api/subscription`, when it would report no access, reconciles directly from the processor and records any subscription that currently grants access — Stripe via `findStripeEntitlement`, RevenueCat via `fetchRevenueCatSubscriber` → `mapRevenueCatSubscriber` (gated on `REVENUECAT_SECRET_API_KEY`, honoring the same sandbox-isolation rule as the webhook). A just-completed purchase therefore activates within the client's post-purchase poll / focus refresh instead of stranding a paying user behind the paywall. Client recovery: the Profile "activating…" card has a "Try again" action and a chat 402 kicks the same self-healing read so a re-send works.
