...
## What just shipped — FX/native-currency storage, snapshot reconstruction & no-cost-questions chat (2026-06)

One session, in-place; one new column (`snapshots.native_breakdown`).

- **FX migration to native-currency display (DONE).** New `convertCurrency(amount, from, to, rates)` pure helper (`src/lib/currency-convert.ts`) computes a single direct cross-rate from the USD-based `fx_rates` table, with an identity short-circuit for same-currency. `snapshots.native_breakdown` (jsonb, per-currency net-worth sums) is now the storage-of-record for the net worth chart, populated by both `writeSnapshot` and `computeRow`; `snapshots.total_value` (USD) remains as a fallback. `formatMoney`, `NetWorthChart`, `useNetWorth`, and `build-inputs.ts`'s `toEur` all moved off the USD double-bridge onto direct cross-rates, with the old double-bridge kept only as a missing-rate fallback in `build-inputs.ts`.
- **`rateAt` gap-fill (DONE).** Backfill's `rateAt` wraps `historicalFxRate` (prior-date carry-forward + live fallback) with an additional later-date forward-fill — only an entirely-empty rate series with no live rate yields `null`.
- **Real estate & mortgage historical reconstruction (DONE).** Each historical snapshot's property value is reconstructed by fitting the CBS regional price index shape (monotone-cubic / PCHIP at fractional-year precision) between two fixed anchors — `buy_price`@`buy_date` and current `value`@today — falling back to linear-in-time when CBS is unresolvable. Mortgage balance gets a matching smooth historical curve via `projectMortgage`, clamped to `[0, grossValue]`. A property contributes nothing before its `buy_date`. Editing a property's mortgage or value triggers a snapshot rebuild from acquisition date forward. See `technical-decisions.md` → "Property Reconstruction (Snapshot History)".
- **Snapshot cadence rule (DONE).** `targetSnapshotDates` is now portfolio-aware: tradeable-holding portfolios keep daily(30d)/weekly(30d–1y)/monthly(>1y); property-only/non-tradeable portfolios get a monthly-only cadence. `writeSnapshot` always writes today regardless.
- **Month-anchored property/FX contributions (DONE).** For every snapshot date, a real estate asset's value, mortgage balance, and FX rate are evaluated at the first of that calendar month (clamped to acquisition date), including the current month — no "today" special case. Other asset types remain exact-date.
- **Chat cost-basis cleanup / no-cost-questions (DONE).** Removed the old upfront cost-basis intake from the chat agent. Tradeable adds now ask only quantity (if ambiguous) and acquisition date (if missing); `getMonthClosingPrice` (Yahoo month-end close) silently fills `buy_price` + `buy_price_source="market"` on commit when a date is given and no price was volunteered. Agent-loop tradeable adds call `commit_mutation` directly once quantity/date are settled, bypassing `propose_mutation`. New `NO_COST_QUESTIONS_BLOCK` prompt block codifies the principle across asset types (see `current-features.md` → "Chat Add Flow — No Cost-Related Questions").
- **Pension intake simplified (DONE).** Capital pensions still ask value/currency/growth/access-age (required, no silent defaults); provider and monthly contribution are optional and recorded only if volunteered. Income pensions ask only annual income/currency; start age is optional via `PENSION_INCOME_AGE_CHIPS` (65/67/68/Skip), defaulting to `DEFAULT_PENSION_ACCESS_AGE` (67) when skipped.
- **Property detail page polish (DONE).** Equity hero restyled to match `NetWorthHero` (uniform serif, no superscript currency); separate PROPERTY card replaced with a single dim metadata line ("of €X value · Y m² · owned since … · X yrs"); "+€X since YEAR" pill removed; section spacing unified across EQUITY/MORTGAGE/INDICATIVE VALUE/Activity.
