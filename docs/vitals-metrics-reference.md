...
- **Letter grades (2026-07, every vitals surface):** `src/lib/vitals/grade.ts` maps
  ...
  solid tone-filled chip with a soft halo (`src/components/GradeChip.tsx`) on
  the mobile fold rail, the desktop Vitals cards, the desktop Overview's
  vitals row, and the marketing mock; all mobile rows rest folded, so the
  chip carries the page's at-a-glance read.
